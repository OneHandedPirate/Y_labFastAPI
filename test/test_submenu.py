...
class TestSubmenu:
    ...
    async def test_submenu_create(self, ac: AsyncClient, prefix, submenu_data):
        resp = await ac.post(f'{prefix}/{pytest.submenu__menu_id}/submenus',
                             json=submenu_data)
        data = resp.json()
    ...
    async def test_submenu_list2(self, ac: AsyncClient, prefix):
        resp = await ac.get(f'{prefix}/{pytest.submenu__menu_id}/submenus')
        data = resp.json()
    ...
    async def test_submenu_delete(self, ac: AsyncClient, prefix):
        resp = await ac.delete(f'{prefix}/{pytest.submenu__menu_id}/submenus/'
                               f'{pytest.submenu__submenu_id}')

        assert resp.status_code == 200

    async def test_submenu_list3(self, ac: AsyncClient, prefix):
        resp = await ac.get(f'{prefix}/{pytest.submenu__menu_id}/submenus')
        data = resp.json()
